...
| 2:30–3:20 | Architecture PNG, then two successful GitHub Actions runs | Reveal Graph left to right; show Runtime READY → HTTP 503 → cleanup, then Memory ACTIVE + Gateway READY → zero events/calls → cleanup | Graph, native hook, and three verified short-lived AgentCore lifecycles |
...
5. Finish on the separate state row: Runtime, Memory, and Gateway were verified only as short-lived lifecycles and then cleaned; the managed trace remains unevidenced.
...
- Show the two successful workflow runs rather than a live AWS console. Together they contain the
  real Runtime READY and HTTP 503 evidence, Memory ACTIVE, Gateway READY and tool list, zero
  events/tool calls, and cleanup without exposing account details.
